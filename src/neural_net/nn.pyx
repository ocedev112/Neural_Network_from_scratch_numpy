...
cdef class NeuralNetwork:
      cdef list weight_matrix
      cdef list bias_matrix
      cdef list layers
      cdef  optimize_weights_activation(self, str activation_sign,int  input_size,int output_size):
      ...
      cdef activation(self, str sign,ndarray[F64, ndim=1] z):
        if sign == "ReLU":
      ...
      def __forward(self,input, weight_matrix, bias_matrix,idx, bint isTraining, training_layers):
          layer_dict = {}
      ...
      def __forward_pass(self,inputs, bint train_mode=False):
           outputs = []
           for input in inputs:
                training_layers = []         
      ...
      cdef derivate_activation(self, str sign, ndarray[F64, ndim=1] a):
           if sign == "ReLU":
           ...
           if sign == "None":
               return 1
      cdef loss_derivative(self,ndarray[F64, ndim=1] predicted_output, ndarray[F64, ndim=1] real_output, str loss):
        if loss == "MSE":
      ...
      cdef regularization(self, str regularizer, matrix,double coef):
           if regularizer == None:
                return 0
           if regularizer == "L1":
      ...
      def compute_loss_adams(self, momentum_w, energy_w, momentum_b, energy_b,  double learning_rate, regularizer,double coef,double ep, int t, double b1,double b2):
       cdef int i
       cdef double b1t = b1 ** t
       cdef double b2t = b2 ** t
      ...
      def train(self, inputs, real_output, training_args=None, eval_dataset=None, check_loss=False):
             ...
             cdef int i, epoch, global_idx, t
             cdef double learning_rate, base_lr, decay, coef
             ...
             if training_args == None:
                  ...
                  if regularizer is not None and regularizer == "L1":
                       coef = training_args.lasso  or 0.0
                  else:
                       coef = training_args.alpha or 0.0
